  ...
  return (
    <div className="w-full py-2">
      {lexicalEntries.map((entry, idx) => {
        return (
          <div className="py-2 px-2 cursor-pointer transition-colors hover:bg-gray-50 rounded-md">
            <div className="flex items-center" key={entry.lexicalCategory.id + idx}>
              <div className="px-2 py-1 rounded-full bg-teal-100 mr-2">{entry.lexicalCategory.id}</div>
              <div>
                {entry.entries.map((it, idx) => {
                  return (
                    <div key={idx}>
                      {it.pronunciations.map(pronunciation => (<div className="flex items-center" key={pronunciation.phoneticNotation}><SpeakerWaveIcon className="h-4 w-4 text-gray-500 mr-1"/>/ {pronunciation.phoneticSpelling} /</div>))}
                    </div>
                  )
                })}
              </div>
